...
function copyTableValueTravelExpense() {
  ...
  if (forInformation_copy != EdocsApi.getAttributeValue("forInformation_copy").value) EdocsApi.setAttributeValue({ code: "forInformation_copy", value: forInformation_copy, text: null });
  ...
  if (currencyEUR.value && dateRate) {
    ...
    if (!response.data) {
      ...
      if (response.data.error) {
        ...
        EdocsApi.setAttributeValue({code: response.data.code, value: Number(response.data.value).toFixed(2), text: null});
      }
    }
  } else {
    if (!currencyEUR.text) {
...
const vutratu = EdocsApi.getAttributeValue("vutratu")
 vutratu.value = Number( EdocsApi.getAttributeValue("rate").value) *Number( vutratuCurrency.value)
 EdocsApi.setAttributeValue(vutratu);
    }
}
}
function onChangetravelDirection () {
    var travelDirectionAttrValue = EdocsApi.getAttributeValue('travelDirection');
    
    if (travelDirectionAttrValue.value =='За кордон') {
